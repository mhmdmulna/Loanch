...
One ERC-20 asset is used for Saver deposits, loans, repayments, and borrower stake. Wallet identity is shared between Saver and Borrower roles. The owner is the trusted MVP verification and configuration authority. There is one active loan per borrower. No personal identity document is stored on-chain.
...
- MVP lending parameters: minimum borrower risk score 60/100, initial reputation 50/100 with minimum eligible 20, loan cap 50% of current Saver principal claims, minimum stake 5% of requested principal rounded up, duration 1–365 days, fixed 10% margin per loan. These fill numeric gaps in the PRD without changing its financial sequence. Duration is seconds in the contract API. Successful completion increases reputation by 5 up to 100; default reduces it by 20 down to zero and permanently blocks new loans in this MVP.
...
| --- | --- | --- |
| Owner | `setIdentityVerification(user, verified)` | Shared wallet verification |
