...
- Transfers use checks, effects, interactions and a reentrancy guard. Deposit, stake, and repayment derive their exact amount from `msg.value`; outgoing BOT transfers revert atomically if the recipient rejects payment.
- Direct BOT transfers to the pool are rejected. Fractional reward dust remains reserved in `saverReturnLiability`; a Saver can claim only whole wei units. No user-count loop runs during repayment or default.
...
| Saver | `deposit() payable` / `withdraw(amount)` | Add or remove principal immediately |
| Saver | `claimReturn()` | Transfer accrued return |
| Borrower | `stake() payable` / `unstake(amount)` | Fund or remove unallocated stake |
| Borrower | `requestLoan(amount, duration)` | Create and disburse one loan atomically |
| Borrower | `repayLoan(loanId) payable` | Pay `msg.value` toward principal then margin |
...
| 4 | Share minting, weights, reserve, payable value accounting |
...
| 10 | Full-system flows, adversarial native receiver, accounting invariants, gas and static analysis |
