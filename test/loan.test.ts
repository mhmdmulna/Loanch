...
describe("LoanchPool loan creation", function () {
  it("allocates stake and disburses once after eligibility succeeds", async function () {
    ...
    const receipt = await (await pool.connect(borrower).requestLoan(400n, 30n * 86400n)).wait();
    ...
    expect(await ethers.provider.getBalance(borrower.address) + receipt!.fee).to.equal(borrowerBefore + 400n);
